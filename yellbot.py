...
def start(update: Update, _: CallbackContext) -> None:
    ...
    update.message.reply_text(
        'Send a picture, a sticker, or some words. Later you can teach me about it with /learn by replying to the sticker I make')
    if yell_tutorial:
...
def callback(update: Update, c: CallbackContext) -> None:
    ...
    if command == "YES" and len(args) == 2:
        identity = args[1]
        result = yelldb.findPending(identity)
        if result:
            (name, fileId, chatId, messageId) = result
            yelldb.learn(name, fileId)
            yelldb.deletePending(identity)
            action = "delete"
            response = "Approved!"
            # print("Learned! ", name)
            ...
            logging.info("%s not found in db", identity)
            action = "delete"
        ...
        logging.info("Unsupported %s %d", command, len(args))

    if action == "delete":
        deleted = c.bot.delete_message(
    ...
    if response:
        send = c.bot.send_message(
            chat_id=chatId, text=response, reply_to_message_id=messageId)
        # print(send)
...
def submitForReview(update: Update, c: CallbackContext, state: yelldb.ChatState):
    ...
    if update.message.from_user and update.message.from_user.id == admin:
        ...
        yelldb.submit(identity, state.input, state.file_id,
                      update.message.chat_id, update.message.message_id)
        fromUser = update.message.from_user and (
        ...
        c.bot.send_document(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(state.input, callback_data="YES " + identity)],
                [InlineKeyboardButton("\u274C " + fromUser,
                                      callback_data="NO " + identity)]
            ]))
...
def learn(update: Update, c: CallbackContext) -> None:
    ...
    if not c.args or len(c.args) == 0:
        if update.message.reply_to_message and update.message.reply_to_message.sticker:
            ...
            if msg.from_user.id == c.bot.id or (msg.forward_from_chat and msg.forward_from_chat.id == log_chan):
                ...
                update.message.reply_text(
                    "Reply with the label you want for this message", reply_to_message_id=state.message_id)
                state.learning = True
                yelldb.saveChatState(state)
            else:
                update.message.reply_text(
                    "If you use /learn, it should be against a sticker I sent!")
        elif state.file_id:
            update.message.reply_text(
                "Reply with the label you want for this message", reply_to_message_id=state.message_id)
            state.learning = True
            ...
            update.message.reply_text(
                "Reply to a sticker I've made with /learn or create a new one by sending me text, a sticker, a photo, or a picture file")
        return
    ...
    if update.message.reply_to_message:
        ...
        update.message.reply_text(
            'Reply to which sticker (from me) you want me to learn')
        ...
        if update.message:
            ...
            if update.message.text:
                ...
                if stickers.validSize(sticker):
                    result = update.message.reply_document(
                        document=open(sticker, "rb"))
                    c.bot.send_document(
                        chat_id=log_chan, document=result.sticker.file_id)
                    logging.info("Sent text sticker: %s", str(result))
                    yelldb.cacheText(update.message.text,
                ...
                result = update.message.reply_sticker(
                ...
                if update.message.forward_from_chat and update.message.forward_from_chat.id == log_chan:
                    ...
                    state.message_id = update.message.message_id
                    state.input = None
                    yelldb.saveChatState(state)
                    return
                ...
                if update.message.sticker.is_animated:
                    ...
                    path = stickers.tempPathExt(
                        update.message.sticker.file_id, "tgs")
                    f.download(path)
                    ...
                    if stickers.validSize(sticker):
                        ...
                        logging.info("Sent sticker %s", str(result))
                        yelldb.cacheFile(
                            update.message.sticker.file_unique_id, result.sticker.file_id)
                    else:
                    ...
                    f = c.bot.get_file(update.message.sticker.file_id)
                    path = stickers.tempPathExt(
                        update.message.sticker.file_id, "webp")
                    f.download(path)
                    img = Image.open(path)
                    wpercent = (max_width/float(img.size[0]))
                    hpercent = (max_height/float(img.size[1]))
                ...
                if doc.mime_type == "image/png" or doc.mime_type == "image/jpeg":
                    ...
                    c.bot.send_document(
                        chat_id=log_chan, document=result.sticker.file_id)
                    logging.info("Sent sticker %s", str(result))
                    yelldb.cacheFile(doc.file_unique_id,
                                     result.sticker.file_id)
                    stickers.deleteSticker(sticker)
                    os.unlink(path)
                    ...
                    if stickers.validSize(sticker):
                        ...
                        update.message.reply_text(
                            "SVG too complex could not import")
                    ...
                    update.message.reply_text(
                        "I can do PNGs and JPEGs, and SVGs but not this")
...
def inlinequery(update: Update, c: CallbackContext) -> None:
    ...
    update.inline_query.answer(
        results, switch_pm_text=switch_pm_text, switch_pm_parameter=switch_pm_parameter)
