...
class CaptchaSays:
    ...
    def textToSticker(self, input_text):
        ...
        style = FontStyle("DejaVu Sans", 60, emoji_svg="emojis/")

        an = objects.Animation(59)
        an.height = 256
        an.width = 256

        x = 0 + (secrets.randbits(8) % 30)
        y = 0 + (secrets.randbits(8) % 30)
        ...
        for index, word in enumerate(words, start=1):
            ...
            color = secrets.choice(colors).strip("#")
            rgb = tuple(int(color[i:i+2], 16) for i in (0, 2, 4))
            layer.add_shape(objects.Fill(Color(rgb[0], rgb[1], rgb[2], 255) / 255))
            layer.add_shape(objects.Stroke(Color(1, 1, 1), 5))
            layer.add_shape(objects.Stroke(Color(0, 0, 0), 10))
            bb = t.bounding_box()
            ...
            if x > 210:
                x = 10 + (secrets.randbits(8) % 30)
                y += bb.height * 2
            act = secrets.randbits(2)
            if act == 0:
                shake(layer.transform.position, 15, 15, 0, 59, 25)
            elif act == 1:
                shake(layer.transform.position, 30, 30, 0, 59, 25)
            elif act == 2:
                rot_shake(layer.transform.rotation, Point((secrets.randbits(8) % 40) - 20, (secrets.randbits(8) % 40) - 20), 0, 60, 10)
