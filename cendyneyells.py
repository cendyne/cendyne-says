...
class CendyneYells:
    ...
    def textToSticker(self, text):
        ...
        if len(text) > 0:
            ...
            if w > 800:
                ...
                layer.add_shape(objects.Stroke(
                    Color(1, 1, 1), width=40 * (0.5 / f)))
            else:
                layer.add_shape(objects.Stroke(Color(1, 1, 1), width=40))
        exporters.export_tgs(an, stickers.tempPath(text), True, False)

    def imageToSticker(self, img, identity):
        sticker = Image.new('RGBA', (512, 512))
        sticker.paste(img)
        sticker.paste(self.img, (0, 0), self.img)
        path = stickers.tempPathExt(identity, "webp")
        sticker.save(path)
        return path

    def animatedToStickerSvg(self, file, identity, max_width, max_height):
        an = self.sticker.clone()
        ...
        l.transform.position.value.y = -1 * \
            bb.y1 * minpercent - (max_height / 4)

        spring_pull(l.transform.position, Point(-1 * bb.x1 * minpercent, -1 *
                    bb.y1 * minpercent + (max_height / 4)), 0, int(an.out_point * 0.5), 30, 7)
        l.transform.position.add_keyframe(int(
            an.out_point * 0.8), Point(-1 * bb.x1 * minpercent, -1 * bb.y1 * minpercent - (max_height / 4)))

        path = stickers.tempPath(identity)
        exporters.export_tgs(an, path, True, False)
        return path

    def animatedToSticker(self, file, identity):
        ...
        yellTime = (self.sticker.out_point -
                    self.sticker.in_point) / self.sticker.frame_rate
        ...
        stickerTime = (sticker.out_point - sticker.in_point) / \
            sticker.frame_rate
        ...
        player.stretch = yellTime / stickerTime * \
            (an.frame_rate / sticker.frame_rate)
    ...
    def makeStickerImg(self, img):
        ...
        return self.imageToSticker(img, identity)

    def makeStickerAnimated(self, file):
        identity = str(uuid.uuid4())
        return self.animatedToSticker(file, identity)

    def makeStickerAnimatedSvg(self, file, max_width, max_height):
        identity = str(uuid.uuid4())
        return self.animatedToStickerSvg(file, identity, max_width, max_height)
